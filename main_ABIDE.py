...
from modules.loader import train_val_test_split_abide, MultiviewData
from modules.utils import print_report_abide
...
parser.add_argument('--indim', type=int, default=206, help='feature dim, #ROIs + #clinical features')
...
parser.add_argument('--load_clf', type=bool, default=False)
...
################## Define Dataloader ##################################
# to generate 'multiview_datasubtype.pt', go to preprocessing/abide_02-process_data.py and run read_multiview_data()
dataset = torch.load(os.path.join(dataroot, 'multiviewdata_miccai.pt'))
...
print("training set")
print_report_abide(dataset=train_dataset)
print("validating set")
print_report_abide(dataset=val_dataset)
print("testing set")
print_report_abide(dataset=test_dataset)
...
model = Model(opt.indim, opt.nclass, opt.nroi, auxdim=6, topk_ratio=opt.ratio,
...
if __name__ == '__main__':
    ...
    model.evaluation_report(data, mode='test', path=None, data_option='ABIDE')

    # TASK2: Visualization on the population graph with four different metrics: label, subtype, site, tr_val_status
    # model.visualize_population_graph(data, choice='label', seed=seed, site=site_choice, data_option='ABIDE')
    # subject_id_cluster = model.visualize_population_graph(data, choice='subtype', seed=seed, site=site_choice)
    # model.visualize_population_graph(data, choice='site', seed=seed, data_option='ABIDE')
